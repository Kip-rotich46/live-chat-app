import { StreamVideo, Call, User } from "@stream-io/video-react-sdk";
...
const MainPage = () => {
  ...
  const joinCall = async(callID: string) =>{
    const call = await client?.call('audio_room', callID);
    try {
      await call?.join()
      setCall(call)
      navigate('/room')
            ...
            <div className="grid">
              {availableRooms.map((room) =>(
                <div className="card" key={room.id} onClick={() => joinCall(room.id)}>
                  ...
                  <p>{room.participantsLength}</p>
                  <p> Created Ny: {room.createdBy}</p>
